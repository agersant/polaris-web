<player>
	...
	<script>
		...
		play(track) {
			...
			fetch("api/lastfm/now_playing/" + track.info.path, { credentials: "same-origin", method: "PUT" });
		...
		updateScrobble() {
			...
			if (this.canScrobble) {
				...
				if (shouldScrobble) {
					fetch("api/lastfm/scrobble/" + this.currentTrack.info.path, { credentials: "same-origin", method: "POST" });
					this.canScrobble = false;
				}
			}
		}
